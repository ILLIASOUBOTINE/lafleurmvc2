    ...
    class Router
	{
		...
		public function findRoute($httpRequest){
			$routeFound = array_filter($this->listRoute,function($route) use ($httpRequest){
				return preg_match("#^" . $route->path . "$#", $httpRequest->getRedirectUrl()) 
				&& $route->method == $httpRequest->getMethod()  
				&& empty(array_diff_assoc($httpRequest->getKeyParam(),$route->param)) 
				&& count($httpRequest->getKeyParam()) == count($route->param); 
			});
